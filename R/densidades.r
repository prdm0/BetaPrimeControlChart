...
densidade_gamma_xbarra <- function(x, mu, beta, n, ...) {
...
densidade_gaussiana_inversa <- function(x, mu, lambda, n, ...) {
  lambda <- n * lambda
  sqrt(lambda / (2 * pi * x^3)) * exp(-lambda * (x - mu)^2 / (2 * mu^2 * x))
}

# integrate(densidade_gamma_xbarra, lower = 0, upper = 10, mu = 0.4, beta = 0.6, n = 200)
# integrate(densidade_gaussiana_inversa, lower = 0, upper = 100, mu = 1, lambda = 1, n = 100)
